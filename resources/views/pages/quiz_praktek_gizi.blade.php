...
@section('content')
    ...
    <section class="content">
        <!-- Default box -->
        <div class="card mt-3" ng-show="!setting">
            <div class="card-header">
                <h3 class="card-title">Jenis Soal</h3>
            </div>
            <div class="card-body">
                <div class="row" >
                    <div class="col-md-12">
                        <button ng-click="create()" type="button" class="btn btn-outline-primary mb-3"><i class="fa fa-plus"></i> Tambah Jenis Soal</button>
        ...
        <div class="card mt-3" ng-show="setting">
            <div class="card-header">
                <h3 class="card-title">Jenis Soal - <% data.title %></h3>
            </div>
            <div class="card-body">
                <button ng-click="kembali()" type="button" class="btn btn-outline btn-secondary mb-3"><i class="fa fa-arrow-left"></i> jenis soal</button>
                <div class="row" >
                    <div class="col-md-12">
                        <div id="list">
        ...
        <div class="modal fade" id="modalform">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <form id="forminput">
                        <input type="text" class="form-control hidden" ng-model="id_jenis" name="id" id="id_jenis">
                        <div class="modal-header">
                            <h4 class="modal-title">Jenis Soal</h4>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" ng-model="nama_jenis" name="title" id="nama">
                                </div>
                            </div>
                            <div class="form-group mb-0">
                                <div class="custom-control custom-checkbox">
                                    <input type="checkbox" id="scoring" name="scoring" ng-model='scoring' ng-checked="scoring=='1'" ng-true-value="'1'" ng-false-value="'0'" class="custom-control-input">
                                    <label class="custom-control-label" for="scoring">Scoring</label>
                                </div>
                            </div>
                            <div class="form-group mb-0">
                                <div class="custom-control custom-checkbox">
                                    <input type="checkbox" id="publish" name="publish" ng-model='publish' ng-checked="publish=='1'" ng-true-value="'1'" ng-false-value="'0'" class="custom-control-input">
...
app.controller("myCtrl", function($scope,$http) {
    ...
    var validator = $("#forminput").validate({
        ...
        submitHandler: function(e){
            ...
            if($scope.update){
                vurl = "{{ url('api/quiz') }}"+"/"+$scope.id_jenis;
                vtype = "PUT";
            }else{
                vurl = "{{ url('api/quiz') }}";
                vtype = "POST";
    ...
    var btnEdit =  " <a class='btn btn-outline-success btn-nav' href='javascript:void(0)' title='Setting' id='SettingRow' ><i class='fa fa-edit tip pointer posdel'></i></a>";
    var btnSoal =  " <a class='btn btn-outline-success btn-nav' href='javascript:void(0)' title='open soal' id='OpenSoal' >soal</a>";
    
    var table =  $('#listdatatable').DataTable({
            columns: [
                { title:"Id" ,    "data":"id" ,     "width":"20px" },
                { title:"Nama",  "data": "title" },
                { title:"Scoring" ,    "data":"scoring" ,     "width":"20px" },
                { title:"Publish" ,    "data":"publish" ,     "width":"20px" },
                { title:"Action", "defaultContent": btnEdit + btnSoal, "width":"100px" },
            ]
    });
    ...
    $('#listdatatable tbody').on('click', '#SettingRow', function () {
        ...
        let data = table.row( tr ).data();
        console.log(data)
        $scope.nama_jenis = data.title
        $scope.id_jenis = data.id
        $scope.publish = data.publish.toString()
        $scope.scoring = data.scoring.toString()
        $scope.update = true
        $('#modalform').modal('show');
        $scope.$apply();
    });
    ...
    $scope.reloadlist = function(){
        Swal.fire({title: 'Loading..',onOpen: () => {Swal.showLoading()}})
        table.clear().draw();
        $http.get("{{ url('api/quiz') }}").then(function(res){
    ...
    $scope.reload = function(id){
        Swal.fire({title: 'Loading..',onOpen: () => {Swal.showLoading()}})
        $http.get("{{ url('api/quizbyid/') }}"+"/"+id).then(function(res){
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    if(data.status){
                        ...
                        $scope.reload($scope.id_jenis_soal);
                    }else{
                        Swal.fire({icon: 'error',title: 'Oops...',text: data.message,})
                    }
                },
